...
try {
	executeInherited(taskName, 'git', ['rebase', 'base/master'])
} catch (err) {
	notify(taskName, 'manual intervention required!')
}
...
while (fs.existsSync(path.join('.', '.git', 'rebase-merge'))) {
...
executePiped(taskName, 'git', ['log', '--no-decorate', '--oneline'])
	...
	.forEach((line) => {
		...
		if (commitMessageToTagNameMap.has(commitMessage)) {
			executeInherited(taskName, 'git', ['tag', '-af', commitMessageToTagNameMap.get(commitMessage), newCommitShortHash])
		}
	})
