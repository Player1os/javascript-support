...
// Process the sub-process's result and handle any encountered error.
const processSubprocessResult = (taskName, command, processArguments, result) => {
	// Check whether the sub-process has failed.
	if (result.status !== 0) {
		// Create the initial message.
		const message = [
			`The sub-process exited with the status code: ${result.statusCode}`,
			` -  command: ${command}`,
		]
...
module.exports.executeInherited = (taskName, command, processArguments = []) => {
	...
	processSubprocessResult(taskName, command, processArguments, result)
}

// Expose the common synchronous command executor with piped stdio, the content of which is returned.
module.exports.executePiped = (taskName, command, processArguments = []) => {
